...
public class SimpleTestWebServer {
	...
	private final class HttpServerSet {
		...
		public HttpServerSet(HttpServer _httpServer,
	...
	public final void setHandler(String path, String response) throws Exception {
		if (path == null || path.isEmpty()) {
			throw new InvalidParameterException(
					"Path must not be blank or null.");
		}
		URL url = URI.create(path).toURL();
		start(url)
				.createContext(url.getPath(), new SimpleHttpHandler(response));
	}
	...
	private final HttpServerSet start(URL url,
			HashMap<String, HttpServerSet> map) throws Exception {
		String baseURI = url.toString().substring(0,
				url.toString().length() - url.getPath().length());
		HttpServerSet httpServerSet = map.get(baseURI);
		if (httpServerSet == null) {
			httpServerSet = new HttpServerSet(HttpServer.create(
					new InetSocketAddress(url.getHost(), url.getPort()),
					url.getPort()), Executors.newFixedThreadPool(1));
